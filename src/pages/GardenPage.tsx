...
const GardenPage = () => {
  const { collectedSeeds, growSeed } = useSeedVerse();
    ...
    <div className="min-h-screen pb-24">
      <div className="gradient-nature-bg pt-10 pb-14 px-4 rounded-b-[3rem] text-center">
        ...
        <p className="text-sm text-primary-foreground/70">
          已收集 {collectedSeeds.length} / {plants.length} 棵植物
        </p>
      </div>
      ...
      <div className="px-4 -mt-8 space-y-4">
        {collectedSeeds.length === 0 ? (
          <motion.div
            ...
            className="card-nature p-8 text-center space-y-3"
          >
            ...
            <Link to="/identify" className="btn-nature inline-block text-sm">
              🔍 去识别植物
            </Link>
          </motion.div>
        ) : (
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-foreground">{plant.emoji} {plant.name}</h3>
